<div class="mx-[4rem] overflow-x-auto pb-[10px]">

    <h1 class="text-4xl font-bold mt-[1rem] mb-[3rem]">PANEL DE CONTROL</h1>

    <div class="flex space-x-2">
        @livewire('componentes.articnuevo')
        ...
        @foreach ($listArticulos as $it)
            <div class="grillas-celdas-2">
                <div class="flex space-x-2 h-[2rem] items-center">
                    @livewire('componentes.articrevisar', ['id' => $it->id], key(uniqid()))
                    @livewire('componentes.articeditar', ['id' => $it->id], key(uniqid()))
                    @livewire('componentes.articpausar', ['id' => $it->id], key(uniqid()))
                    @livewire('componentes.articeliminar', ['id' => $it->id], key(uniqid()))
                </div>
            </div>
            ...
            <div class="grillas-celdas-2 justify-end">{{ $it->precio }}</div>
